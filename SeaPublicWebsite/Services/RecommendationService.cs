        ...
        public static List<Recommendation> GetRecommendationsForUser(UserDataModel userData)
        {
            ...
            {
                {
                    "A", new Recommendation
                    {
                        ...
                        Summary =
                            "Increase the level of insulation in your loft to the recommended level of 300mm"
                    }
                },
                {
            ...
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    var wsse_header = string.Join("", "WSSE UsernameToken ", "Token=\"", token, "\", ", "Nonce=\"",
                        nonce,
                        "\", ",
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        foreach (var recommendation in recommendations)
                            Console.WriteLine($"{recommendation.Title}: {recommendation.Summary}");
