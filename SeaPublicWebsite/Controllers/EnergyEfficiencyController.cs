...
public class EnergyEfficiencyController : Controller
{
    ...
    public async Task<IActionResult> NewOrReturningUser_Post(NewOrReturningUserViewModel viewModel)
    {
        ...
        if (viewModel.NewOrReturningUser is NewOrReturningUser.ReturningUser)
        {
            ...
            {
                ModelState.AddModelError(nameof(NewOrReturningUserViewModel.Reference),
                    sharedLocalizer["Check you have typed the reference correctly. Reference must be 8 characters."]);
                return NewOrReturningUser_Get();
            }

            return await ReturningUser_Get(viewModel.Reference, false);
        }
    ...
    public async Task<IActionResult> AskForPostcode_Post(AskForPostcodeViewModel viewModel, bool cancel = false)
    {
        ...
        {
            ModelState.AddModelError(nameof(AskForPostcodeViewModel.Postcode),
                sharedLocalizer["Enter a valid UK postcode"]);
        }
        ...
        var forwardArgs = GetActionArgumentsForQuestion(
            ...
            extraRouteValues: new Dictionary<string, object>
                { { "postcode", viewModel.Postcode }, { "number", viewModel.HouseNameOrNumber } });
    ...
    public async Task<IActionResult> CheckYourUnchangeableAnswers_Get(string reference,
    ...
    public async Task<IActionResult> NoRecommendations_Get(string reference, string emailAddress = null,
    ...
    public async Task<IActionResult> NoRecommendations_Post(NoRecommendationsViewModel viewModel)
    {
        if (!ModelState.IsValid)
        {
            return await NoRecommendations_Get(viewModel.Reference, viewModel.EmailAddress);
        }
    ...
    public async Task<IActionResult> GenerateRecommendationsPdf_Post(string reference)
    {
        ...
        var stream =
            await pdfGenerationService.GeneratePdf(
                $"energy-efficiency/pdf-generation/your-recommendations/{encodedReference}");
    ...
    public async Task<IActionResult> GenerateActionPlanPdf_Post(string reference)
    {
        var encodedReference = HttpUtility.UrlEncode(reference);
        var stream =
            await pdfGenerationService.GeneratePdf($"energy-efficiency/pdf-generation/action-plan/{encodedReference}");
        ...
        string reference)
    {
        ...
        if (command == "goBackwards")
        {
            return RedirectToAction(nameof(Recommendation_Get),
                new
                {
                    id = (int)propertyData.GetPreviousRecommendationKey(recommendationKey),
                    reference = propertyData.Reference
                });
        }
        ...
        if (command == "goForwards")
        {
            ...
            return RedirectToAction(nameof(Recommendation_Get),
                new
                {
                    id = (int)propertyData.GetNextRecommendationKey(recommendationKey),
                    reference = propertyData.Reference
                });
        }
    ...
    public async Task<IActionResult> ActionPlan_Post(YourSavedRecommendationsEmailViewModel viewModel)
    {
        if (!ModelState.IsValid)
        {
            return await ActionPlan_Get(viewModel.Reference, viewModel.EmailAddress);
        }
        ...
        catch (EmailSenderException e)
        {
            switch (e.Type)
            {
                ...
                case EmailSenderExceptionType.Other:
                    ModelState.AddModelError(nameof(emailAddress),
                        sharedLocalizer[
                            "Unable to send email due to unexpected error. Please make a note of your reference code."]);
                    return;
                default:
                    throw new ArgumentOutOfRangeException();
            }
    ...
    private async Task<IActionResult> RenderActionPlanView(string reference, bool isPdf)
    {
        ...
        {
            BackLink = Url.Action(nameof(Recommendation_Get),
                new { id = (int)propertyData.GetLastRecommendationKey(), reference }),
            PropertyData = propertyData,
            EmailAddress = null,
            EmailSent = false,
            IsPdf = isPdf,
    ...
    private RedirectToActionResult RedirectToNextStep(QuestionFlowStep nextStep, string reference,
        QuestionFlowStep? entryPoint = null)
    {
        ...
        IDictionary<string, object> extraRouteValues = null)
    {
        return question switch
        {
            ...
            QuestionFlowStep.NewOrReturningUser => new PathByActionArguments(nameof(NewOrReturningUser_Get),
                "EnergyEfficiency"),
            QuestionFlowStep.OwnershipStatus => new PathByActionArguments(nameof(OwnershipStatus_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.Country => new PathByActionArguments(nameof(Country_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.FindEpc => new PathByActionArguments(nameof(FindEpc_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.ServiceUnsuitable => new PathByActionArguments(nameof(ServiceUnsuitable),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.AskForPostcode => new PathByActionArguments(nameof(AskForPostcode_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.ConfirmAddress => new PathByActionArguments(nameof(ConfirmAddress_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.ConfirmEpcDetails => new PathByActionArguments(nameof(ConfirmEpcDetails_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.NoEpcFound => new PathByActionArguments(nameof(NoEpcFound_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.PropertyType => new PathByActionArguments(nameof(PropertyType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HouseType => new PathByActionArguments(nameof(HouseType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.BungalowType => new PathByActionArguments(nameof(BungalowType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.FlatType => new PathByActionArguments(nameof(FlatType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HomeAge => new PathByActionArguments(nameof(HomeAge_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.CheckYourUnchangeableAnswers => new PathByActionArguments(
                nameof(CheckYourUnchangeableAnswers_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.WallConstruction => new PathByActionArguments(nameof(WallConstruction_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.CavityWallsInsulated => new PathByActionArguments(nameof(CavityWallsInsulated_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.SolidWallsInsulated => new PathByActionArguments(nameof(SolidWallsInsulated_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.FloorConstruction => new PathByActionArguments(nameof(FloorConstruction_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.FloorInsulated => new PathByActionArguments(nameof(FloorInsulated_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.RoofConstruction => new PathByActionArguments(nameof(RoofConstruction_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.SolarElectricPanels => new PathByActionArguments(nameof(SolarElectricPanels_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.LoftSpace => new PathByActionArguments(nameof(LoftSpace_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.LoftAccess => new PathByActionArguments(nameof(LoftAccess_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.RoofInsulated => new PathByActionArguments(nameof(RoofInsulated_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.OutdoorSpace => new PathByActionArguments(nameof(OutdoorSpace_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.GlazingType => new PathByActionArguments(nameof(GlazingType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HeatingType => new PathByActionArguments(nameof(HeatingType_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.OtherHeatingType => new PathByActionArguments(nameof(OtherHeatingType_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HeatingControls => new PathByActionArguments(nameof(HeatingControls_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HotWaterCylinder => new PathByActionArguments(nameof(HotWaterCylinder_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.NumberOfOccupants => new PathByActionArguments(nameof(NumberOfOccupants_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.HeatingPattern => new PathByActionArguments(nameof(HeatingPattern_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.Temperature => new PathByActionArguments(nameof(Temperature_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues, entryPoint)),
            QuestionFlowStep.AnswerSummary => new PathByActionArguments(nameof(AnswerSummary_Get), "EnergyEfficiency",
                GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.NoRecommendations => new PathByActionArguments(nameof(NoRecommendations_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues)),
            QuestionFlowStep.YourRecommendations => new PathByActionArguments(nameof(YourRecommendations_Get),
                "EnergyEfficiency", GetRouteValues(reference, extraRouteValues)),
    ...
    private async Task<List<string>> GetDuplicateExpiredEpcIds(IEnumerable<EpcSearchResult> searchResults)
    {
        ...
        var epcDtoPairTaskList = duplicates.Select(async duplicate =>
            new KeyValuePair<string, EpbEpcAssessmentDto>(duplicate.EpcId,
                await epcApi.GetEpcDtoForId(duplicate.EpcId)));
