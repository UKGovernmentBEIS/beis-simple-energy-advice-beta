...
public class EpcTests
{
        ...
        new (string Descrption, string[] inputWallsDescription, WallConstruction? outputWallConstruction)[]
            {
                ...
                ("Can parse wall construction solid (sandstone or limestone)", new[] { "Sandstone or limestone" },
                    WallConstruction.Solid),
                ("Can parse wall construction solid (granite or whinstone)", new[] { "Granite or whinstone" },
                    WallConstruction.Solid),
                ("Can parse wall construction solid (solid and sandstone)",
                    new[] { "solid brick", "Sandstone or limestone" }, WallConstruction.Solid),
                ("Can parse wall construction cavity", new[] { "cavity wall" }, WallConstruction.Cavity),
                ("Can parse wall construction mixed walls", new[] { "solid brick", "cavity wall" },
                ...
                ("Can parse wall construction other (Park home wall)", new[] { "Park home wall" },
                    WallConstruction.Other),
                ("Can parse wall construction other (other + solid)", new[] { "Cob", "solid brick" },
                    WallConstruction.Other)
            }
            .Select(p => new EpcTestCase(
                p.Descrption,
    ...
    private static readonly EpcTestCase[] SolidWallsInsulatedTestCases =
        new (string Descrption, string[] inputWallsDescription, SolidWallsInsulated? outputSolidWallsInsulated)[]
            {
                ("Can parse solid walls insulated", new[] { "solid brick, as built, insulated (assumed)" },
                    SolidWallsInsulated.All),
                ("Can parse solid walls insulated (internal)", new[] { "solid brick, with internal insulation" },
                    SolidWallsInsulated.All),
                ("Can parse solid walls insulated (external)", new[] { "solid brick, with external insulation" },
                    SolidWallsInsulated.All),
                ("Can parse solid walls partially insulated",
                    new[] { "solid brick, as built, partial insulation (assumed)" }, SolidWallsInsulated.Some),
                ("Can parse solid walls no insulation", new[] { "solid brick, as built, no insulation (assumed)" },
                    SolidWallsInsulated.No)
            }
            .Select(p => new EpcTestCase(
                p.Descrption,
                new EpbEpcAssessmentDto
    ...
    private static readonly EpcTestCase[] CavityWallsInsulatedTestCases =
        new (string Descrption, string[] inputWallsDescription, CavityWallsInsulated? outputCavityWallsInsulated)[]
            {
                ("Can parse cavity walls insulated", new[] { "cavity wall, as built, insulated (assumed)" },
                    CavityWallsInsulated.All),
                ("Can parse cavity walls insulated (filled cavity)", new[] { "cavity wall, filled cavity" },
                    CavityWallsInsulated.All),
                ("Can parse cavity walls insulated (internal)", new[] { "cavity wall, with internal insulation" },
                    CavityWallsInsulated.All),
                ("Can parse cavity walls insulated (external)", new[] { "cavity wall, with external insulation" },
                    CavityWallsInsulated.All),
                ("Can parse cavity walls partially insulated",
                    new[] { "cavity wall, as built, partial insulation (assumed)" }, CavityWallsInsulated.Some),
                ("Can parse cavity walls no insulation", new[] { "cavity wall, as built, no insulation (assumed)" },
                    CavityWallsInsulated.No)
            }
            .Select(p => new EpcTestCase(
                p.Descrption,
                new EpbEpcAssessmentDto
        ...
        new (string Descrption, string[] inputFloorDescription, FloorConstruction? outputFloorConstruction)[]
            {
                ...
                ("Can parse floor construction suspended timber", new[] { "suspended" },
                    FloorConstruction.SuspendedTimber),
                ("Can parse floor construction suspended timber (suspended + suspended)",
                    new[] { "suspended", "suspended" }, FloorConstruction.SuspendedTimber),
                ("Can parse floor construction solid concrete", new[] { "solid" }, FloorConstruction.SolidConcrete),
                ("Can parse floor construction solid concrete (solid + solid)", new[] { "solid", "solid" },
                    FloorConstruction.SolidConcrete),
        ...
        new (string Descrption, string[] inputFloorDescription, FloorInsulated? outputFloorInsulation)[]
            {
                ...
                ("Can parse floor insulated (limited)", new[] { "solid, limited insulation (assumed)" },
                    FloorInsulated.Yes),
        ...
        new (string Descrption, string[] inputRoofDescription, RoofInsulated? outputRoofInsulated)[]
            {
                ...
                ("Can parse roof insulated (insulated at rafters)", new[] { "pitched, insulated at rafters" },
                    RoofInsulated.Yes),
                ("Can parse roof insulated (limited)", new[] { "pitched, limited insulation (assumed)" },
                    RoofInsulated.Yes),
        ...
        new (string Descrption, string[] inputWindowsDescription, GlazingType? outputGlazingType)[]
            {
                ...
                ("Can parse glazing type both (partial secondary)", new[] { "partial secondary glazing" },
                    GlazingType.Both),
                ("Can parse glazing type both (partial multiple)", new[] { "partial multiple glazing" },
                    GlazingType.Both),
                ...
                ("Can parse glazing type both (mostly secondary)", new[] { "mostly secondary glazing" },
                    GlazingType.Both),
                ("Can parse glazing type both (mostly multiple)", new[] { "mostly multiple glazing" },
                    GlazingType.Both),
                ("Can parse glazing type both (throughout)", new[] { "multiple glazing throughout" }, GlazingType.Both),
                ("Can parse glazing type double or triple (full secondary)", new[] { "full secondary glazing" },
                    GlazingType.DoubleOrTripleGlazed),
                ("Can parse glazing type double or triple (fully double)", new[] { "fully double glazed" },
                    GlazingType.DoubleOrTripleGlazed),
                ("Can parse glazing type double or triple (fully triple)", new[] { "fully triple glazed" },
                    GlazingType.DoubleOrTripleGlazed),
                ("Can parse glazing type double or triple (high)", new[] { "High performance glazing" },
                    GlazingType.DoubleOrTripleGlazed)
            }
            .Select(p => new EpcTestCase(
                p.Descrption,
    ...
    private static readonly EpcTestCase[] HeatingTypeTestCases =
        new (string Descrption, string inputMainFuelType, string inputMainHeatingDescription, EpcHeatingType?
                outputEpcHeatingType)[]
                {
                    ("Can handle null heating type", null, null, null),
                    ("Can parse heating type other (mains gas (community))", "mains gas (community)", null,
                        EpcHeatingType.Other),
                    ...
                    ("Can parse heating type other (electricity (community))", "electricity (community)", null,
                        EpcHeatingType.Other),
                    ("Can parse heating type other (waste combustion (community))", "waste combustion (community)",
                        null, EpcHeatingType.Other),
                    ("Can parse heating type other (biomass (community))", "biomass (community)", null,
                        EpcHeatingType.Other),
                    ("Can parse heating type other (biogas (community))", "biogas (community)", null,
                        EpcHeatingType.Other),
                    ("Can parse heating type other (heat from boilers that can use mineral oil or biodiesel (community))",
                        "heat from boilers that can use mineral oil or biodiesel (community)", null,
                        EpcHeatingType.Other),
                    ("Can parse heating type other (heat from boilers using biodiesel from any biomass source (community))",
                        "heat from boilers using biodiesel from any biomass source (community)", null,
                        EpcHeatingType.Other),
                    ("Can parse heating type other (biodiesel from vegetable oil only (community))",
                        "biodiesel from vegetable oil only (community)", null, EpcHeatingType.Other),
                    ("Can parse heating type other (from heat network data (community))",
                        "from heat network data (community)", null, EpcHeatingType.Other),
                    ("Can parse heating type other (biodiesel from any biomass source)",
                        "biodiesel from any biomass source", null, EpcHeatingType.Other),
                    ("Can parse heating type other (biodiesel from used cooking oil only)",
                        "biodiesel from used cooking oil only", null, EpcHeatingType.Other),
                    ("Can parse heating type other (biodiesel from vegetable oil only (not community))",
                        "biodiesel from vegetable oil only (not community)", null, EpcHeatingType.Other),
                    ...
                    ("Can parse heating type other (wood pellets in bags for secondary heating)",
                        "wood pellets in bags for secondary heating", null, EpcHeatingType.Other),
                    ("Can parse heating type gas boiler (mains gas - this is for backwards compatibility only and should not be used)",
                        "mains gas - this is for backwards compatibility only and should not be used", null,
                        EpcHeatingType.GasBoiler),
                    ("Can parse heating type gas boiler (biogas - landfill)", "biogas - landfill", null,
                        EpcHeatingType.GasBoiler),
                    ("Can parse heating type gas boiler (mains gas (not community))", "mains gas (not community)", null,
                        EpcHeatingType.GasBoiler),
                    ("Can parse heating type gas boiler (biogas (not community))", "biogas (not community)", null,
                        EpcHeatingType.GasBoiler),
                    ("Can parse heating type LPG boiler (LPG - this is for backwards compatibility only and should not be used)",
                        "LPG - this is for backwards compatibility only and should not be used", null,
                        EpcHeatingType.LpgBoiler),
                    ("Can parse heating type LPG boiler (bottled LPG)", "bottled LPG", null, EpcHeatingType.LpgBoiler),
                    ("Can parse heating type LPG boiler (LPG special condition)", "LPG special condition", null,
                        EpcHeatingType.LpgBoiler),
                    ("Can parse heating type LPG boiler (LPG (not community))", "LPG (not community)", null,
                        EpcHeatingType.LpgBoiler),
                    ("Can parse heating type oil boiler (oil - this is for backwards compatibility only and should not be used)",
                        "oil - this is for backwards compatibility only and should not be used", null,
                        EpcHeatingType.OilBoiler),
                    ("Can parse heating type oil boiler (B30K (not community))", "B30K (not community)", null,
                        EpcHeatingType.OilBoiler),
                    ("Can parse heating type oil boiler (oil (not community))", "oil (not community)", null,
                        EpcHeatingType.OilBoiler),
                    ("Can parse heating type oil boiler (appliances able to use mineral oil or liquid biofuel)",
                        "appliances able to use mineral oil or liquid biofuel", null, EpcHeatingType.OilBoiler),
                    ("Can parse heating type coal or solid fuel (anthracite)", "anthracite", null,
                        EpcHeatingType.CoalOrSolidFuel),
                    ("Can parse heating type coal or solid fuel (house coal)", "house coal", null,
                        EpcHeatingType.CoalOrSolidFuel),
                    ("Can parse heating type coal or solid fuel (smokeless coal)", "smokeless coal", null,
                        EpcHeatingType.CoalOrSolidFuel),
                    ("Can parse heating type coal or solid fuel (house coal (not community))",
                        "house coal (not community)", null, EpcHeatingType.CoalOrSolidFuel),
                    ("Can parse heating type biomass boiler (wood logs)", "wood logs", null, EpcHeatingType.Biomass),
                    ("Can parse heating type biomass boiler (bulk wood pellets)", "bulk wood pellets", null,
                        EpcHeatingType.Biomass),
                    ("Can parse heating type biomass boiler (wood chips)", "wood chips", null, EpcHeatingType.Biomass),
                    ("Can parse heating type biomass boiler (dual fuel)", "dual fuel - mineral + wood", null,
                        EpcHeatingType.Biomass),
                    ("Can parse heating type biomass boiler (biomass)", "biomass", null, EpcHeatingType.Biomass),
                    ("Can parse heating type other (electricity)", "electricity", "community heating system",
                        EpcHeatingType.Other),
                    ("Can parse heating type direct action electric (boiler with radiators)", "electricity",
                        "boiler with radiators or underfloor heating", EpcHeatingType.DirectActionElectric),
                    ("Can parse heating type direct action electric (underflow heating)", "electricity",
                        "electric underfloor heating", EpcHeatingType.DirectActionElectric),
                    ("Can parse heating type direct action electric (warm air system)", "electricity",
                        "warm air system (not heat pump)", EpcHeatingType.DirectActionElectric),
                    ("Can parse heating type direct action electric (room heaters)", "electricity", "room heaters",
                        EpcHeatingType.DirectActionElectric),
                    ("Can parse heating type direct action electric (other)", "electricity", "other system",
                        EpcHeatingType.DirectActionElectric),
                    ("Can parse heating type heat pump", "electricity",
                        "heat pump with radiators or underfloor heating", EpcHeatingType.HeatPump),
                    ("Can parse heating type heat pump", "electricity", "heat pump with warm air distribution",
                        EpcHeatingType.HeatPump),
                    ("Can parse heating type storage heater", "electricity", "electric storage heaters",
                        EpcHeatingType.Storage),
                    ("Can parse heating type gas boiler (electricity + micro-cogeneration)", "electricity",
                        "micro-cogeneration", EpcHeatingType.GasBoiler)
                }
            .Select(p => new EpcTestCase(
                p.Descrption,
                new EpbEpcAssessmentDto
    ...
    private static readonly EpcTestCase[] SolarElectricPanelsTestCases =
        new (string Descrption, int? inputSolarElectricPanelCoverage, SolarElectricPanels? outputSolarElectricPanels)[]
            {
                ("Can handle null solar electric panels", null, null),
                ("Can parse has solar electric panels (0)", 0, SolarElectricPanels.No),
                ("Can parse has solar electric panels (1)", 1, SolarElectricPanels.Yes),
                ("Can parse has solar electric panels (50)", 50, SolarElectricPanels.Yes),
                ("Can parse has solar electric panels (100)", 100, SolarElectricPanels.Yes)
            }
            .Select(p => new EpcTestCase(
                p.Descrption,
                new EpbEpcAssessmentDto
                {
                    AssessmentType = "RdSAP",
                    SolarElectricPanelPercentRoofCoverage = p.inputSolarElectricPanelCoverage
                },
                new Epc
                {
